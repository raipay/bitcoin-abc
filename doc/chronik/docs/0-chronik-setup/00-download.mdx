...
{/* Read current Bitcoin ABC version so we can generate the latest link */}
import CMakeLists from "!!raw-loader!../../../../CMakeLists.txt"
export const regexVersion = /VERSION (\d+\.\d+.\d+)/;
export const version = regexVersion.exec(CMakeLists)[1];
...
## Windows
Chronik is currently not built by default for Windows, but you can [build it from source yourself](build-chronik).
