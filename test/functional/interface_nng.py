...
class NngInterfaceTest(BitcoinTestFramework):
    ...
    def _get_utxo(self, node):
        ...
        vout = 0
        value = int(coinbase_tx['vout'][vout]['value'] * XEC)
    ...
    async def _check_undo_slice(self, rpc_sock, block_file_num, tx, coins_raw):
        from NngInterface import GetUndoSliceResponse
        await self._send_request(
            rpc_sock, self._make_get_undo_slice_request_fbb(block_file_num, tx.UndoPos(), tx.UndoSize()))
        response = await self._recv_response(rpc_sock)
    ...
    async def _test_genesis(self, node, rpc_sock):
        ...
        for params in [{'height': 0}, {'blockhash': bytes.fromhex(rpc_genesis_blockhash)[::-1]}]:
            ...
            assert_equal(block_tx.DataPos(), 89)
            assert_equal(block_tx.UndoPos(), 0)
            assert_equal(block_tx.UndoSize(), 0)
    ...
    async def _test_send_tx(self, node, rpc_sock):
        ...
        assert_equal(block.Txs(0).DataPos(), 21172)
        assert_equal(block.Txs(0).UndoPos(), 0)
        assert_equal(block.Txs(0).UndoSize(), 0)
        assert_equal(block.Txs(1).DataPos(), 21272)
        assert_equal(block.Txs(1).UndoPos(), 4519)
        assert_equal(block.Txs(1).UndoSize(), 25)
        assert_equal(block.Txs(2).DataPos(), 21372)
        assert_equal(block.Txs(2).UndoPos(), 4519 + 25)
        ...
        undo_data = bytes.fromhex('0103003201da1745e9b549bd0bfa1a569971c77eba30cd5a4b')
        await self._check_undo_slice(rpc_sock, block.FileNum(), block.Txs(1), undo_data)
        undo_data = bytes.fromhex('01805e0094b9c93a0169d7ef8f42a25e8791bb37d5fb48456f102a3cb9')
        await self._check_undo_slice(rpc_sock, block.FileNum(), block.Txs(2), undo_data)
        
        assert_equal(block.Txs(1).Tx().SpentCoinsLength(), 1)
        spent_coin = block.Txs(1).Tx().SpentCoins(0)
        assert_equal(spent_coin.TxOut().Amount(), amount)
        assert_equal(get_fb_bytes(spent_coin.TxOut(), 'Script').hex(), self.anyone_script)
        assert_equal(spent_coin.IsCoinbase(), True)
        assert_equal(spent_coin.Height(), 1)
        ...
        assert_equal(spent_coin.TxOut().Amount(), amount - 1000)
        assert_equal(get_fb_bytes(spent_coin.TxOut(), 'Script').hex(), self.anyone_script2)
        assert_equal(spent_coin.IsCoinbase(), False)
    ...
    async def _test_transaction_added_to_mempool(self, node, pub_sock):
        ...
        assert_equal(spent_coins.TxOut().Amount(), 50_000_000_00)
    ...
    async def _test_block_connected(self, node, pub_sock):
        ...
        assert_equal(spent_coin.TxOut().Amount(), 50_000_000_00)
        assert_equal(get_fb_bytes(spent_coin.TxOut(), 'Script').hex(), self.anyone_script)
        assert_equal(spent_coin.IsCoinbase(), True)
