...
def __plugin_outputs__(tx: Tx):
    counter = 0
    plugin_data = tx.inputs[0].plugin_data
    if plugin_data is not None:
        if "test_plugin" in plugin_data:
            counter = int.from_bytes(plugin_data["test_plugin"].data[0], "big") + 1
    return [
        {"idx": 1, "data": [counter.to_bytes(4, "big")], "group": b"group"}
    ]
"""
        )
        plugin_file.close()
        ...
        assert_equal(
            ...
            {"test_plugin": pb.PluginEntry(groups=[b"group"], data=[b"\0\0\0\0"])},
        )
        assert_equal(
        ...
        # Spending the tx provides the plugin_data, which increments the counter
        tx2 = CTransaction()
        tx2.vin = [
            CTxIn(
        ...
        proto_tx = chronik.tx(txid2).ok()
        assert_equal(
            proto_tx.outputs[1].plugins,
            {"test_plugin": pb.PluginEntry(groups=[b"group"], data=[b"\0\0\0\x01"])},
        )
