...
const useWallet = () => {
    ...
    const deriveAccount = async ({ masterHDNode, path }) => {
        ...
        const cashAddress = cashaddr.encode('ergon', 'P2PKH', node.identifier);
    ...
    */
    ...
    const fetchBchPrice = async (
        fiatCode = cashtabSettings ? cashtabSettings.fiatCurrency : 'usdt',
    ) => {
        ...
        const priceApiUrl = `https://api.xeggex.com/api/v2/market/getbysymbol/${cryptoId}_${fiatCode}t`;
        let bchPrice;
        try {
        ...
        try {
            const bchPriceJson = await bchPrice.json();
            let bchPriceInFiat = parseFloat(bchPriceJson.lastPrice);

            const validergonPrice = typeof bchPriceInFiat === 'number';
