    ...
    */
    ...
    for (let i = 0; i < contactList.length; i += 1) {
        ...
        if (
            ...
            if (
                isValidXrgAddress(contactObj.address) &&
...
export const isValidXrgAddress = addr => {
    /* 
    Returns true for a valid XRG address

    Valid XRG address:
    - May or may not have prefix `Ergon:`
    - Checksum must validate for prefix `ergo,:`
    
    A simple ledger address is not considered a valid Ergon address
    */
    ...
    let isValidXrgAddress;
    let isPrefixedXrgAddress;

    // Check for possible prefix
    if (addr.includes(':')) {
        // Test for 'ecash:' prefix
        isPrefixedXrgAddress = addr.slice(0, 6) === 'ergon:';
        console.log(isPrefixedXrgAddress)
        // Any address including ':' that doesn't start explicitly with 'Ergon:' is invalid
        if (!isPrefixedXrgAddress) {
            isValidXrgAddress = false;
            return isValidXrgAddress;
        }
    } else {
        isPrefixedXrgAddress = false;
    }

    // If no prefix, assume it is checksummed for an ergon: prefix
    const testedXecAddr = isPrefixedXrgAddress ? addr : `ergon:${addr}`;

    try {
        const decoded = cashaddr.decode(testedXecAddr);
        if (decoded.prefix === 'ergon') {
            isValidXrgAddress = true;
        }
    } catch (err) {
        isValidXrgAddress = false;
    }
...
export const isValidAirdropExclusionArray = airdropExclusionArray => {
    ...
    for (let i = 0; i < addressStringArray.length; i++) {
        if (!isValidXrgAddress(addressStringArray[i])) {
            return false;
        }
    }
