...
describe('Validation utils', () => {
    ...
    it(`isValidXrgAddress correctly validates a valid XRG address with ergon: prefix`, () => {
        const addr = 'ergon:qzvydd4n3lm3xv62cx078nu9rg0e3srmqqkm80dnl6';
        expect(isValidXrgAddress(addr)).toBe(true);
    });
    it(`isValidXrgAddress correctly validates a valid XRG address without ergon: prefix`, () => {
        const addr = 'qzvydd4n3lm3xv62cx078nu9rg0e3srmqqkm80dnl6';
        expect(isValidXrgAddress(addr)).toBe(true);
    });
    it(`isValidXrgAddress rejects a valid legacy address`, () => {
        const addr = '1Efd9z9GRVJK2r73nUpFmBnsKUmfXNm2y2';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects a valid ecash: address`, () => {
        const addr = 'ecash:qz2708636snqhsxu8wnlka78h6fdp77ar59jrf5035';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects a valid ecash: address without the ecash prefix`, () => {
        const addr = 'qz2708636snqhsxu8wnlka78h6fdp77ar59jrf5035';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects a valid etoken: address with prefix`, () => {
        const addr = 'etoken:qz2708636snqhsxu8wnlka78h6fdp77ar5tv2tzg4r';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects a valid etoken: address without prefix`, () => {
        const addr = 'qz2708636snqhsxu8wnlka78h6fdp77ar5tv2tzg4r';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects a valid simpleledger: address with prefix`, () => {
        const addr = 'simpleledger:qrujw0wrzncyxw8q3d0xkfet4jafrqhk6csev0v6y3';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects a valid simpleledger: address without prefix`, () => {
        const addr = 'qrujw0wrzncyxw8q3d0xkfet4jafrqhk6csev0v6y3';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects an invalid address`, () => {
        const addr = 'wtf is this definitely not an address';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects a null input`, () => {
        const addr = null;
        expect(isValidXrgAddress(addr)).toBe(false);
    });
    it(`isValidXrgAddress rejects an empty string input`, () => {
        const addr = '';
        expect(isValidXrgAddress(addr)).toBe(false);
    });
