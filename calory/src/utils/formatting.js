...
export const formatBalance = (unformattedBalance, optionalLocale) => {
    try {
        // Determine the number of actual decimal places in unformattedBalance
        const decimalPlaces = (unformattedBalance.toString().split('.')[1] || '').length;
        
        // Set the number of fraction digits to display
        const fractionDigits = Math.min(decimalPlaces, appConfig.cashDecimals);

        const formattedBalance = new Number(unformattedBalance).toLocaleString(optionalLocale || undefined, {
