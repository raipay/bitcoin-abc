...
use pyo3::{
    types::{PyDict, PyFunction, PyList, PyModule},
...
impl PluginContext {
    ...
    /// loading plugins.
    pub fn setup(conf: PluginConfig) -> Result<Option<Self>> {
...
version = platform.python_version()
    ...
    fn load_plugin(
        ...
        let Ok(py_lokad_id) = module.getattr("__lokad_id__") else {
            log!(
                "Warning: Plugin {module_name} has no __lokad_id__ set, skipping\n"
            );
            return Ok(());
        };
        ...
        let Ok(py_version) = module.getattr("__version__") else {
            log!(
                "Warning: Plugin {module_name} has no __version__ set, skipping\n"
            );
            return Ok(());
        };
        let Ok(version) = py_version.extract::<String>() else {
            log!(
                "Warning: Plugin {module_name} __version__ must be str, got {py_version}, skipping\n"
            );
            return Ok(());
        };
        ...
        let Some(lokad_ids) = lokad_ids else {
            log!(
                "Warning: Plugin {module_name} has invalid __lokad_id__, must be 4 bytes\n"
            );
            return Ok(());
        };
        ...
        let Ok(fn_plugin_outputs) = module.getattr("__plugin_outputs__") else {
            log!(
                "Warning: Plugin {module_name} has no __plugin_outputs__ function defined, skipping\n"
            );
            return Ok(());
        };
    ...
    pub fn run_plugin_outputs<'ctx, 'py>(
        ...
        let tx = self.tx_module.bridge_tx(py, tx, token_data)?;
        let mut result = PluginResult::default();

        // TODO: filter tx again by plugin LOKAD ID
        for plugin in &self.plugins {
            let plugin_call_result =
                plugin.fn_plugin_outputs.call1(py, (tx.clone_ref(py),));
            ...
            let Ok(outputs) = outputs.downcast::<PyList>() else {
                log!("Plugin {} returned unexpected result {outputs}, skipping\n", plugin.name);
                continue;
            };
            for (idx, output) in outputs.iter().enumerate() {
                let Ok(output) = output.downcast::<PyDict>() else {
                    log!("Plugin {} returned unexpected list entry at index {idx}: {output}, skipping\n", plugin.name);
                    continue;
                };
                let Some(output_idx) = output.get_item("idx")? else {
                    log!("Plugin {} returned dict with missing \"idx\" at index {idx}, skipping\n", plugin.name);
                    continue;
                };
                let Ok(output_idx) = output_idx.extract::<usize>() else {
                    log!("Plugin {} returned dict with invalid \"idx\" at index {idx}: {output_idx}, expected int, skipping\n", plugin.name);
                    continue;
                };
                let Some(groups) = output.get_item("group")? else {
                    log!("Plugin {} returned dict with missing \"group\" at index {idx}, skipping\n", plugin.name);
                    continue;
                };
                let Ok(groups) = extract_bytes_or_list(groups) else {
                    log!("Plugin {} returned dict with invalid \"group\" at index {idx}: {groups}, expected bytes or iterable of bytes, skipping\n", plugin.name);
                    continue;
                };
                let mut data = vec![];
                if let Some(py_data) = output.get_item("data")? {
                    let Ok(py_data) = extract_bytes_or_list(py_data) else {
                        log!("Plugin {} returned dict with invalid \"data\" at index {idx}: {py_data}, expected bytes or iterable of bytes, skipping\n", plugin.name);
                        continue;
                    };
                    data = py_data;
                }
                ...
                match entry {
                    Entry::Occupied(_) => {
                        log!("Plugin {} returned a duplicate entry for output index {output_idx}, skipping\n", plugin.name);
                        continue;
                    }
                    Entry::Vacant(vacant) => {
    ...
    /// Fast pre-filter whether the tx matches any of the LOKAD IDs of any plugin.
