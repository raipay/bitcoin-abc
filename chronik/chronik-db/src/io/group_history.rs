...
pub struct GroupHistoryMemData {
    /// In-memory cache for tx history.
...
pub struct GroupHistoryStats {
    ...
    /// Number of batch queries for the member's number of txs.
    pub n_num_txs_batch_get: usize,
}
...
impl<'a, G: Group> GroupHistoryWriter<'a, G> {
    ...
    fn fetch_members_num_txs<'tx>(
        ...
        let GroupHistoryMemData { cache, stats } = mem_data;
        ...
        let mut members_num_txs = vec![None; ser_members.len()];
        if cache.num_txs_cache.is_some() {
            for (member_num_txs, member_ser) in
                members_num_txs.iter_mut().zip(&ser_members)
            {
                if let Some(num_txs) =
                    cache.get_num_txs_cache(member_ser.as_ref())
                {
                    stats.n_num_txs_cache_hit += 1;
                    *member_num_txs = Some(num_txs);
                }
            }
        }

        let t_fetch = Instant::now();
        let missing_num_txs_keys = members_num_txs
            .iter()
            .zip(&ser_members)
            .filter_map(|(num_txs, member_ser)| match num_txs {
                None => Some(member_ser.as_ref()),
                _ => None,
            });
        let batch_num_txs = self
            .col
            .db
            .multi_get(self.col.cf_num_txs, missing_num_txs_keys, true)
            .collect::<Result<Vec<_>>>()?;
        stats.t_fetch += t_fetch.elapsed().as_secs_f64();
        stats.n_num_txs_batch_get += batch_num_txs.len();

        for (member_num_txs, db_num_txs) in members_num_txs
            .iter_mut()
            .filter(|num_txs| num_txs.is_none())
            .zip(batch_num_txs)
        {
            *member_num_txs = Some(
                db_num_txs
                    .map(|db_num_txs| bytes_to_num_txs(&db_num_txs))
                    .transpose()?
                    .unwrap_or(0),
            );
        }

        let members_num_txs = members_num_txs
            .into_iter()
            .map(|num_txs| num_txs.unwrap())
            .collect::<Vec<_>>();
...
impl GroupHistoryMemData {
    /// Memdata with cache enabled and set to num_txs_cache_size entries.
    pub fn cached(num_txs_cache_size: usize) -> Self {
        GroupHistoryMemData {
            stats: Default::default(),
            cache: GroupHistoryCache {
                num_txs_cache: num_txs_cache_size
                    .try_into()
                    .ok()
                    .map(LruCache::new),
            },
    ...
    /// Memdata with cache disabled.
    pub fn uncached() -> Self {
        GroupHistoryMemData::default()
    }
}
...
mod tests {
    ...
    fn test_value_group_history() -> Result<()> {
        ...
        let mem_data = RefCell::new(GroupHistoryMemData::cached(20));
