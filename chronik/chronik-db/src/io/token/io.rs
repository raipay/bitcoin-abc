...
impl<'a> TokenWriter<'a> {
    ...
    pub fn upgrade_11_to_12(
            ...
            self.col.db.full_iterator(self.col.cf_token_tx).enumerate()
        {
            if db_idx % 10000 == 0 {
                log!(
                    "Scanned {db_idx}/{estimated_num_keys} (estimated) token txs\n"
                );
            }
            ...
            // If a token tx only has NoToken outputs, it may have the wrong input tokens, so we need to reload them
