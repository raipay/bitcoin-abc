...
use bitcoinsuite_core::tx::{OutPoint, TxOutput};
use bytes::Bytes;
...
use crate::{
    ...
    plugins::data::{PluginOutput, PluginOutputEntry},
...
pub struct PluginGroup<'a> {
    /// Groups assigned to outputs by plugins
    pub plugin_outputs: &'a BTreeMap<OutPoint, PluginOutput>,
}

static EMPTY_OUTPUTS: BTreeMap<OutPoint, PluginOutput> = BTreeMap::new();
impl<'a> Default for PluginGroup<'a> {
    fn default() -> Self {
        PluginGroup {
            plugin_outputs: &EMPTY_OUTPUTS,
        }
    }
}

impl<'b> Group for PluginGroup<'b> {
    ...
    fn input_members<'a>(&self, query: GroupQuery<'a>) -> Self::Iter<'a> {
        ...
        for (input_idx, input) in query.tx.inputs.iter().enumerate() {
            let Some(plugin_output) = self.plugin_outputs.get(&input.prev_out)
            else {
                continue;
            };
    ...
    fn output_members<'a>(&self, query: GroupQuery<'a>) -> Self::Iter<'a> {
        ...
        for output_idx in 0..query.tx.outputs.len() {
            ...
            let Some(plugin_output) = self.plugin_outputs.get(&outpoint) else {
                continue;
            };
            for (plugin_name, entry) in &plugin_output.plugins {
                for group in &entry.groups {
...
impl<'a> PluginsWriter<'a> {
    ...
    pub fn insert(
        ...
        let plugin_history_writer =
            GroupHistoryWriter::new(self.col.db, &group)?;
        ...
        let plugin_utxo_writer = GroupUtxoWriter::new(self.col.db, &group)?;
    ...
    pub(crate) fn add_cfs(columns: &mut Vec<rocksdb::ColumnFamilyDescriptor>) {
        ...
        GroupHistoryWriter::<PluginGroup<'_>>::add_cfs(columns);
        GroupUtxoWriter::<PluginGroup<'_>>::add_cfs(columns);
    }
}
