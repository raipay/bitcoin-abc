...
impl<'a, BI: BitcoindInterface> Txs<'a, BI> {
    ...
    pub(crate) fn rich_block_tx(&self, tx_num: TxNum, block_tx: &BlockTx) -> Result<RichTx> {
        ...
        let raw_tx = self.indexer.bitcoind_interface.get_block_slice(
            block.file_num,
        ...
        let spent_coins = match block_tx.entry.undo_pos {
            0 => None,
            _ => {
                let undo_data = self.indexer.bitcoind_interface.get_undo_slice(
                    block.file_num,
