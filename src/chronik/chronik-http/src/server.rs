...
pub type SlpIndexerRef<BI> = Arc<RwLock<SlpIndexer<BI>>>;

pub struct ChronikServer<BI> {
    pub addr: SocketAddr,
    pub slp_indexer: SlpIndexerRef<BI>,
}
...
pub enum ChronikServerError {
    ...
    #[critical()]
    #[error("Unexpected error from JSONRPC: {0}")]
    BitcoindJsonRpcError(String),
}
...
impl<BI: BitcoindInterface> ChronikServer<BI> {
    pub async fn run(self) -> Result<(), Report> {
        let addr = self.addr;
        let app = Router::new()
            .route(
                "/broadcast-tx",
                routing::post(handle_broadcast_tx::<BI>).on(MethodFilter::OPTIONS, handle_post_options),
            )
            .route(
                "/broadcast-txs",
                routing::post(handle_broadcast_txs::<BI>).on(MethodFilter::OPTIONS, handle_post_options),
            )
            .route("/blockchain-info", routing::get(handle_blockchain_info::<BI>))
            .route("/blocks/:start/:end", routing::get(handle_blocks::<BI>))
            .route("/block/:hash_or_height", routing::get(handle_block::<BI>))
            .route("/tx/:txid", routing::get(handle_tx::<BI>))
            .route("/raw-tx/:txid", routing::get(handle_raw_tx::<BI>))
            .route("/token/:token_id", routing::get(handle_token::<BI>))
            .route(
                "/script/:type/:payload/history",
                routing::get(handle_script_history::<BI>),
            )
            .route(
                "/script/:type/:payload/utxos",
                routing::get(handle_script_utxos::<BI>),
            )
            .route(
                "/validate-utxos",
                routing::post(handle_validate_utxos::<BI>).on(MethodFilter::OPTIONS, handle_post_options),
            )
            .route("/ws", routing::get(handle_subscribe::<BI>))
...
impl<BI> Clone for ChronikServer<BI> {
    fn clone(&self) -> Self {
        Self {
            addr: self.addr,
            slp_indexer: self.slp_indexer.clone(),
        }
    }
}
...
async fn handle_broadcast_tx<BI: BitcoindInterface>(
    Protobuf(broadcast_request): Protobuf<proto::BroadcastTxRequest>,
    Extension(server): Extension<ChronikServer<BI>>,
...
async fn handle_broadcast_txs<BI: BitcoindInterface>(
...
async fn handle_blockchain_info<BI: BitcoindInterface>(
    Extension(server): Extension<ChronikServer<BI>>,
...
async fn handle_blocks<BI: BitcoindInterface>(
...
async fn handle_block<BI: BitcoindInterface>(
    ...
    let block_header_json = slp_indexer.bitcoind_rpc()
        .run_rpc_command("getblockheader", &[&block.hash.to_string()])?
        .map_err(BitcoindJsonRpcError)?;
...
async fn handle_tx<BI: BitcoindInterface>(
...
async fn handle_raw_tx<BI: BitcoindInterface>(
    Path(txid): Path<String>,
    Extension(server): Extension<ChronikServer<BI>>,
...
async fn handle_token<BI: BitcoindInterface>(
...
async fn handle_script_history<BI: BitcoindInterface>(
...
async fn handle_script_utxos<BI: BitcoindInterface>(
    Path((script_type, payload)): Path<(String, String)>,
    Extension(server): Extension<ChronikServer<BI>>,
...
async fn handle_validate_utxos<BI: BitcoindInterface>(
...
async fn handle_subscribe<BI: BitcoindInterface>(
    ws: WebSocketUpgrade,
    Extension(server): Extension<ChronikServer<BI>>,
) -> impl IntoResponse {
...
async fn handle_subscribe_socket<BI: BitcoindInterface>(
    mut socket: WebSocket,
    server: ChronikServer<BI>,
) {
