...
struct MitraAmountFormatter {
    template <typename Stream>
    void Ser(Stream &s, const Amount &amount) const {
        NumFmt numFmt;
...
template <typename NumFmt>
struct MitraCOutPointFormatter {
    template <typename Stream>
...
template <typename NumFmt>
struct MitraCTxInFormatter {
    FORMATTER_METHODS(CTxIn, obj) {
        READWRITE(Using<MitraCOutPointFormatter<NumFmt>>(obj.prevout),
                  Using<MitraVectorFormatter<DefaultFormatter, NumFmt>>(obj.scriptSig),
                  Using<NumFmt>(obj.nSequence));
    }
};

template <typename NumFmt>
struct MitraCTxOutFormatter {
    FORMATTER_METHODS(CTxOut, obj) {
        READWRITE(Using<MitraAmountFormatter<NumFmt>>(obj.nValue),
                  Using<MitraVectorFormatter<DefaultFormatter, NumFmt>>(obj.scriptPubKey));
    }
};

template <class TxType, typename NumFmt>
struct MitraCTxFormatter {
    FORMATTER_METHODS(TxType, obj) {
        READWRITE(obj.nVersion,
                  Using<MitraVectorFormatter<MitraCTxInFormatter<NumFmt>, NumFmt>>(obj.vin),
                  Using<MitraVectorFormatter<MitraCTxOutFormatter<NumFmt>, NumFmt>>(obj.vout),
                  Using<NumFmt>(obj.nLockTime));
    }
};

template <typename NumFmt>
struct MitraCTxRefFormatter {
...
template <typename NumFmt>
struct MitraCBlockFormatter {
    FORMATTER_METHODS(CBlock, obj) {
        READWRITEAS(CBlockHeader, obj);
        READWRITE(Using<VectorFormatter<MitraCTxRefFormatter<NumFmt>>>(obj.vtx));
    }
};
